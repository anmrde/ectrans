...
#endif
#ifdef OMPGPU
!$OMP TARGET ENTER DATA MAP(TO:ZAA,ZAS,IZBS,IZCST,DZBST,DZCST,DZCAT)
!$OMP TARGET ENTER DATA MAP(TO:F,F%RN,F%RLAPIN,S,S%FA,S%ITHRESHOLD,S%LUSEFLT,D,D%NUMP,D%MYMS,R,R%NDGNH,R%NSMAX,G,G%NDGLU)
!$OMP TARGET ENTER DATA MAP(TO:D%NPNTGTB0,D%NPNTGTB1,D%NSTAGT0B,D%NSTAGT1B,D%NSTAGTF,G%NMEN,D%NPROCM,D%NPTRLS,G,G%NLOEN,D%MSTABF)
#endif
...
!$OMP TARGET ENTER DATA MAP(TO:G_NLOEN_MAX,F_RW)
#endif
