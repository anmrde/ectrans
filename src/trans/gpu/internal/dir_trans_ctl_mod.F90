...
#endif
#ifdef OMPGPU
     !$OMP TARGET DATA MAP(TO:FOUBUF_IN)
#endif
...
#endif
#ifdef OMPGPU
  !$OMP TARGET DATA MAP(FROM:PSPSC2)    IF(KF_SCALARS > 0 .AND. PRESENT(PSPSC2) .AND. NF_SC2 > 0)
#endif
#ifdef ACCGPU
  !$ACC DATA COPYOUT(PSPSC3A)   IF(KF_SCALARS > 0 .AND. PRESENT(PSPSC3A) .AND. NF_SC3A > 0)
#endif
#ifdef OMPGPU
  !$OMP TARGET DATA MAP(FROM:PSPSC3A)   IF(KF_SCALARS > 0 .AND. PRESENT(PSPSC3A) .AND. NF_SC3A > 0)
#endif
#ifdef ACCGPU
  !$ACC DATA COPYOUT(PSPSC3B)   IF(KF_SCALARS > 0 .AND. PRESENT(PSPSC3B) .AND. NF_SC3B > 0)
#endif
#ifdef OMPGPU
  !$OMP TARGET DATA MAP(FROM:PSPSC3B)   IF(KF_SCALARS > 0 .AND. PRESENT(PSPSC3B) .AND. NF_SC3B > 0)
#endif
