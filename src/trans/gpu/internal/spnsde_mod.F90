...
DO KMLOC=1,D%NUMP
  ...
  !$ACC&   PRESENT(KM,ISMAX,F,ZN,ZZEPSNM,PEPSNM,KMLOC)
#endif
  DO JN=KM-1,ISMAX+2
   IJ = ISMAX+3-JN
   ZN(IJ) = F%RN(JN)
  ...
  IF(KM == 0) THEN
      ...
      !$ACC&    PRESENT(KF_SCALARS,ISMAX,PNSD,KMLOC,ZN,ZZEPSNM,PF)
#endif
      DO J=1,KF_SCALARS
        IR = 2*J-1
        DO JI=2,ISMAX+3
  ...
  ELSE  
    ...
    !$ACC&    PRESENT(KF_SCALARS,ISMAX,KM,PNSD,KMLOC,ZN,ZZEPSNM,PF)
#endif
    DO J=1,KF_SCALARS
      DO JI=2,ISMAX+3-KM
        IR = 2*J-1
